(function () {
  ...
  function imageThumb(src, title, type = "image") {
    ...
    return `<div class="admin-thumb-mark"><img src="assets/logo.jpeg" alt="" /></div>`;
  }
  ...
  const woltReaderPrefix = "https://r.jina.ai/http://r.jina.ai/http://";
  ...
  async function importFromWoltLink() {
    ...
    try {
      ...
      if (url.includes("/venue/alkopoint") && window.AlcoPointWoltProducts?.length) {
        imported.push(
  ...
  async function careerPayload(form) {
    ...
    return {
      ...
      title: field(form, "title").value.trim(),
      description: field(form, "description").value.trim(),
      image: uploaded?.url || field(form, "image").value.trim() || "assets/logo.jpeg",
  ...
  function bindActions() {
    ...
    document.getElementById("logoutBtn").addEventListener("click", () => setAuthed(false));
